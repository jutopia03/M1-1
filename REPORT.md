...
- **그래프에서 보이는 것**:
  - 일별 원본에는 요일 주기와 개봉 일정에 따른 노이즈가 크게 섞여 있어 트렌드가 육안으로 보이지 않는다. 30일 이동평균으로 노이즈를 걷어내자 성수기·비수기가 반복되는 계절성과 완만한 상승 트렌드가 함께 드러난다.
...
- **사용 기법**: 전일 대비 변화율 + 1위 점유율. 변화율은 절대 규모와 무관하게 "어느 날 갑자기 시장이 움직였는가"를 찾아내므로, 대작 개봉처럼 사건이 일어난 날을 특정하는 데 적합하다.
- **그래프에서 보이는 것**:
